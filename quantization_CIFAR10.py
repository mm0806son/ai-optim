'''
@Name           :quantization_CIFAR10.py
@Description    :
@Time           :2022/03/21 15:27:11
@Author         :Zijie NING
@Version        :1.0
'''
import sys
...
if device == "cuda":
    model_fp = torch.nn.DataParallel(model_fp)
...
model_fp.load_state_dict(checkpoint["model"])
...
if args.optim == "SGD":
    optimizer = optim.SGD(model_fp.parameters(), lr=args.lr, momentum=0.9, weight_decay=1e-3)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=200)
elif args.optim == "Adam":
    optimizer = optim.Adam(model_fp.parameters(), lr=args.lr, weight_decay=1e-3)
else:
